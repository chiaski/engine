...
const mapControls = {
  ...
  */
  ...
  initMap: function (x, y) {
    ...
    for (let i = 0; i < globals.MAP_height; i++) {
      for (let j = 0; j < globals.MAP_width; j++) {
        if (x !== null && y !== null && i == x && j == y) {
          console.log("b");
          map_html += `<div class='_s __active __start' data-active='active' data-scene='${i},${j}'><span>${i},${j}</span></div>`;
          ...
          map_html += `<div class='_s __unused' data-active='unused'  data-scene='${i},${j}'><span>${i},${j}</span></div>`;
        }
      }
      map_html += "<hr>";
    }
    ...
    setTimeout(function(){ sceneControls.switchScene(x,y); }, 250);
    ...
    for (let i = 0; i < globals.MAP_height; i++) {
      for (let j = 0; j < globals.MAP_width; j++) {
        ...
        if (x !== null && y !== null && i == x && j == y) {
          ...
          map_html += `<div class='_s __active __start' data-active='active' data-scene='${i},${j}' style='background:${c};'><span>${i},${j}</span></div>`;
            
//            ("<div class='_s __active __start' data-active='active' data-scene='" + i + "," + j + "' style='background:" + c + ";'>" + i + "," + j + "</div>");
...
//            "<div class='_s __active __start' data-active='active' data-scene='" + i + "," + j + "' style='background:" + c + ";'>" + i + "," + j + "</div>";
          ...
          if (data.s[sceneControls.getSceneIndex(i, j)] == null) {
            // unused scene
            map_html += `<div class='_s __unused' data-active='unused' data-scene='${i},${j}'><span>${i},${j}</span></div>`   
//              "<div class='_s __unused' data-active='unused'  data-scene='" + i + "," + j + "'>" + i + "," + j + "</div>";
          } else {
            c = data.s[sceneControls.getSceneIndex(i, j)].color;
            
            map_html += `<div class='_s __inactive' data-active='inactive' data-scene='${i},${j}' style='background:${c};'><span>${i},${j}</span></div>`;
              
//              "<div class='_s __inactive' data-active='inactive' data-scene='" + i + "," + j + "' style='background:" + c + ";'>" + i + "," + j + "</div>";
          }
        }
      }
      map_html += "<hr>";
...
$(document).on("click", "[action='sceneSetStarting']", function(){
  
  if (!$("[action='sceneSetStarting']").is(':checked')) return;
  
  if(!confirm(`Change your starting scene to ${active_scene.x}, ${active_scene.y}?`)){
    $("[action='sceneSetStarting']").prop("checked", false).removeAttr("checked");
    $("[action='sceneDelete']").removeAttr("disabled");
    return;
  }
  
  let x = active_scene.x;
  let y = active_scene.y;
  
    mapControls.clearMapStart();
    $("#" + mapControls.MASTER + " ._s[data-scene='" + x + "," + y + "']").addClass("__start");
    $("#" + mapControls.SIDE + " ._s[data-scene='" + x + "," + y + "']").addClass("__start");

    scenes.start_scene.x = x;
    scenes.start_scene.y = y;

    
});

// adjust map starting scene
//
