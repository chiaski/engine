  ...
  */
  ...
  loadScene: function (x, y) {
    ...
    ((Tplayer.active).objects).forEach(function (e) {
      ...
      $(newObj).hide().fadeIn(1200).appendTo("#e-play");

    })

    // update text
    $("#play [text='scene']").text((Tplayer.active).x + "," + (Tplayer.active).y);
  ...
  loadCartridge: function () {
    ...
    if(e.size){  $(newObj).css("width", e.size).css("height", e.size); }
      ...
      $(newObj).hide().fadeIn(2000).appendTo("#e-cartridge");
    });
  ...
  $("#play h2").html("<span>Starting <span text='cartridge-title'>game</span>...</span>").fadeIn("slow");
    
  if(scenes.title) $("[text='cartridge-title']").html(scenes.title);
    
  Tplayer.active = scenes.s[sceneControls.getSceneIndex(x, y)];
    
  // Is there a song?
  if (scenes.audio !== null) Tplayer.playSong();
  ...
  // change text
  setTimeout(function () {
    $("#play h2").html("<span text='scene-title'></span><span text='scene'></span>")
...
$("#btn-clear").on("click", function () {
  Tplayer.clearGame();
  Tplayer.loadCover();
  $("#btn-play").html("Play & Preview");
  $("#btn-clear").css("opacty", 0.5);
});
