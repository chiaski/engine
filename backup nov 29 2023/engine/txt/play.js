...
const PLAY = {
  ...
  start: function(){
    ...
    $("[onMode='player'] .--directions").on("click", "a[side]", function(){
  ...
  loadScene: function(x, y){
    ...
    $("[onMode='player'] .--directions a").removeAttr("active").attr("inactive", "");
    ...
    for (var _a in a.actions) {
        if ((a.actions).hasOwnProperty(_a)) {
         console.log((a.actions)[_a]);
          ...
          $("div[onMode='player'] a[side='" + (a.actions)[_a].dir +"']")
            .removeAttr("inactive").attr("active", "");

          $("div[onMode='player'] a[side='" + (a.actions)[_a].dir +"']  span[text]").html((a.actions)[_a].text);
        }
    }
